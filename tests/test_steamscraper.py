...
@pytest.fixture
def mock_config():
    """Mock SteamConfig for testing."""
    config = MagicMock(spec=SteamConfig)
    config.appid = ARK_SURVIVAL_EVOLVED_APPID
    config.userid = "test_userid"
    config.password = "test_password"
    config.username = "test_user"
    config.steamid = "76561197971835915"
    return config
...
class TestSteamScraper:
    ...
    def test_init_with_credentials(self, mock_webauth, mock_config):
        """Test initialization with configuration."""
        scraper = SteamScraper(config=mock_config)

        # Verify WebAuth was called with the correct username
        assert scraper._config.appid == ARK_SURVIVAL_EVOLVED_APPID  # Default app ID
        mock_webauth.login.assert_called_once_with(password="test_password")

    # The prompt functionality is now handled by the SteamConfig class, so this test is no longer needed

    def test_init_with_custom_appid(self, mock_webauth, mock_config):
        """Test initialization with custom app ID."""
        # Create a new mock config with custom appid
        custom_config = mock_config
        custom_config.appid = "123456"

        scraper = SteamScraper(config=custom_config)

        # Verify app ID was set correctly
        assert scraper._config.appid == "123456"

    def test_parse_userid(self, mock_webauth, mock_config):
        ...
        # Set the steam_id property on the mock_webauth
        mock_webauth.steam_id = mock_config.steamid

        scraper = SteamScraper(config=mock_config)
        url1 = f'https://steamcommunity.com/id/{scraper._user.steam_id}/myworkshopfiles/?appid={scraper._config.appid}&browsefilter=mysubscriptions'
        response = scraper._user.session.get(url1)
        result = response.url
        ...
        mock_webauth.session.get.assert_called_with(
            f'https://steamcommunity.com/id/{mock_webauth.steam_id}/myworkshopfiles/?appid={mock_config.appid}&browsefilter=mysubscriptions'
        )

    def test_parse_data_empty(self, mock_webauth, mock_bs4, mock_config):
    ...
    def test_parse_data_with_results(self, mock_webauth, mock_bs4, mock_config):
    ...
    def test_get_data(self, mock_webauth, monkeypatch, mock_config):
        """Test get_data method."""
        # Mock the _parse_userid and _parse_data methods
        ...
        # Patch the methods
        monkeypatch.setattr(scraper, "_parse_userid", mock_parse_userid)
        monkeypatch.setattr(scraper, "_parse_data", mock_parse_data)
        ...
        # Verify _parse_userid was called
        mock_parse_userid.assert_called_once()

        # Verify _parse_data was called 3 times (for 3 pages)
        ...
        expected_calls = [
            f'https://steamcommunity.com/id/12345/myworkshopfiles/&p=1&numberpage=30',
            f'https://steamcommunity.com/id/12345/myworkshopfiles/&p=2&numberpage=30',
            f'https://steamcommunity.com/id/12345/myworkshopfiles/&p=3&numberpage=30',
        ]
